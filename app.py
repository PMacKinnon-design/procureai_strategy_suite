...
""")
    ...
    if st.button("📄 Generate AI Strategy Summary Report"):
        with st.spinner("Generating report with GPT-4..."):
            ...
            # Show in-app with tighter spacing
            cleaned_html = report_text.replace(chr(10), "<br>").replace("<br><br>", "<br>")
            st.markdown(f"<div style='font-family: Helvetica; font-size: 15px; line-height: 1.4;'>{cleaned_html}</div>", unsafe_allow_html=True)
            ...
            st.download_button(
                ...
                data=pdf_bytes,
                file_name="MacKinnon_AI_Strategy_Report.pdf",
