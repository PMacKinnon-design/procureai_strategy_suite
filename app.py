    ...
    with st.spinner("Analyzing uploaded files..."):
        ...
        for sheet, df in ai_readiness_df.items():
            ...
            if "Score (1–5)" in df.columns:
                scores = df["Score (1–5)"].dropna().astype(float)
                ...
                ai_data.append({
                    ...
                    "Comments Summary": " | ".join(comments)
                })
        if all_scores:
            ai_data.append({
        ...
        st.write("🧪 DEBUG: ai_results content")
st.json(ai_data)
ai_results_df = pd.DataFrame(ai_data, columns=["Category", "Avg Score", "Comments Summary"])
    ...
    if st.button("Generate AI Strategy Report"):
        with st.spinner("Generating strategy report..."):
            report_text = generate_strategy_narrative(kpi_df, questionnaire_df, client, ai_data)
