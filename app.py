...
if uploaded_file:
    ...
    # Load benchmark data including source
    ...
    # Merge for display with unit and source
    unit_map = {
        "Cost Savings %": "%",
        "Contract Coverage": "%",
        "PO Cycle Time": "days",
    ...
    table_df = pd.merge(
        ...
        benchmark_df[["Metric", "Benchmark", "Source"]],
        on="Metric",
        how="outer"
    )
    ...
    st.subheader("📊 Metrics vs Benchmark Analysis Table (with Sources)")
    display_cols = ["Metric", "Value", "Benchmark", "Difference", "Unit", "Source"]
