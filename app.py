...
st.markdown("<h1 style='color: #1F4E79;'>ProcureAI Strategy Suite (Grouped Charts by Unit)</h1>", unsafe_allow_html=True)
st.markdown("### ℹ️ You are running the **unit-grouped table + chart version**.")
...
if uploaded_file:
    ...
    # Group by unit and show individual tables + charts
    for unit in table_df["Unit"].dropna().unique():
        ...
        if not unit_df.empty:
            ...
            ax.barh(unit_df["Metric"], unit_df["Benchmark"], label="Benchmark", color="#ff7f0e", alpha=0.6)
            ax.barh(unit_df["Metric"], unit_df["Value"], label="Your Value", color="#1f77b4")
