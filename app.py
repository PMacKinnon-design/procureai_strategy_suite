...
# Parse AI Readiness with debug visibility
if ai_file:
    st.subheader("🔍 DEBUG: Reading AI Readiness Sheets")
    sheet_data = pd.read_excel(ai_file, sheet_name=None)
    st.write("Sheet Names:", list(sheet_data.keys()))
    parsed_data = []

    for category, df in sheet_data.items():
        st.write(f"📄 Sheet: {category}")
