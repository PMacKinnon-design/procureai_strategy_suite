...
st.set_page_config(layout="wide", page_title="ProcureAI Strategy Suite (Table First)")
st.markdown("<h1 style='color: #1F4E79;'>ProcureAI Strategy Suite (Table-First Approach)</h1>", unsafe_allow_html=True)

client = OpenAI(api_key=st.secrets.get("OPENAI_API_KEY", ""))  # avoid crash if not set
...
if uploaded_file:
    ...
    st.subheader("📊 Table: Metrics vs Benchmark Analysis")
    ...
    # This sets us up for charting later (can reuse table_df)
